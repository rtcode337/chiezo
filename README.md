...
| 固める | 覚えたことのうち確定したものを、テーマごとに読み取り専用のソースへ焼く(短期記憶 → 長期記憶)。焼いた先はほかのソースと同じ口で引ける |

人間が中身を確かめるための簡易ブラウズ画面と管理画面(`/admin`)も持つ。
画面では、ためた知識と固めた知識を**長期記憶**、覚えたことを**短期記憶**と呼んで節を分けている。

知識ベースとしての中身は上の 4 つ。それとは別に「使う」側も同梱している。Chiezo を
...
覚えたことのうち確定したもの(決まりごと、環境の事実…)は、テーマごとに読み取り専用の
ソースへ焼ける。焼いた先は普通のソースなので `/v1/<テーマ名>/search` で引けるし、
短期記憶を空けても中身は残る。

```bash
curl -s "$BASE/v1/memory/themes" -H 'Content-Type: application/json' \
  -d '{"name":"rules","label":"決まりごと","tags":"rule"}'   # テーマを定義する
# あとは管理画面の「固化する」→「片付ける」(取り込みと同じ経路で焼かれる)
```

素材は**前世代の長期記憶 + 未固化のメモ**なので、追加も更新も削除も短期記憶に書くだけで
反映される(削除は同じ見出しに `削除` タグ)。焼き直しは毎回の作り直しなので、
失敗しても前の世代へ戻せる。
