...
chiezo を引いて、質問へ日本語で答えます。**Claude Code から MCP / curl で引くのと同じ知識を、
ローカルの LLM からも使うための口**です。根拠にした文書は出典として併記します。
...
- `grounded` — 回答方針。`1`(既定)は chiezo の抜粋だけを根拠にし、根拠が無ければ答えません。
