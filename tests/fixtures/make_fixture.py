...
ARTICLES = [
    ...
    {
        # カテゴリにソートキーが付いた記事(`[[Category:X|よみがな]]`)の回帰テスト用。
        # この書き方だと本文(strip_code)側にはソートキーしか残らず、カテゴリ名
        # 「日本の小説家」が本文から消える。本文の全文検索で "Category:" 行を拾う
        # 方式ではこの記事を取りこぼす(実例: ラーメン二郎)。tags は wikitext の
        # リンク先から取るので落ちず、filter?tag= なら引ける。
        "id": 5,
        "title": "夏目漱石",
