...
# 脚注・出典タグ: mwparserfromhell の strip_code は タグの中身を本文として残すため、
# 消さないと注釈や出典の題名が地の文に流れ込む
# (「NIFRELは、大阪府吹田市千里万博公園内所在地の実際は、…万博記念公園の中に所在する」)。
...
# 名前付き引数で座標を持つテンプレート(日本の駅・空港・会社の Infobox 等)の引数名。
# (度, 分, 秒, 方位) の順。秒・方位は省略されることがある。
#
# `基礎情報 会社` は接頭辞付きの名前を使う(`本社緯度度` / `本店緯度度`)。
# 素の `緯度度` だけを見ていると、店・メーカーの記事の座標がまとめて落ちる
# (手元の飲食・食品の記事では、度分秒を持つ 30 件のうち 19 件が落ちていた)。
# 本社を先に見て、空なら本店を使う —— 会社の代表点は本社だが、飲食店の記事では
# 本社が空で本店だけ埋まっていることがある(中華四川)。
_NAMED_COORD_KEYS = [
...
def _extract_coordinates(code) -> tuple[float, float] | None:
    ...
    ja Wikipedia の座標は {{Coord}} 系テンプレートのほか、駅・空港・施設・会社の Infobox が
    持つ `緯度度`/`本社緯度度` のような名前付き引数にも入っている。両方を拾う。
...
def _drop_non_prose(code) -> None:
    ...
    `strip_code()` は タグの中身を本文として残すので、`<ref>` を消さないと注釈や
