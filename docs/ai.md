...
同じ名前の依頼は管理画面の「見比べ」(`/admin/media`)に 1 組として並び、依頼文つきで
...
- 画面は一覧(`/admin/media`)と中身(`/admin/media/{組の名前}`)に分かれています。
  一覧が運ぶのは見出し・依頼日時・種類・件数までで、案の中身は開いたときだけ ——
  依頼が溜まるほど、開くつもりのないものまで毎回運ぶことになるため(音は 1 本で数 MB)
- 画面からも頼めます(「作ってもらう」の欄)。**会話できない相手(自前の GPU など)に
  頼む道はここだけ**です
...
**認証の失敗(401)が返った相手は、Chiezo が自分で無効にします。** 401 は認証情報を
登録し直すまで直らないので、有効のまま残すとワーカーがそこへ振り続け、頼むたびに
同じ失敗を積むためです。無効にした相手はワーカーの振り先からも外れ、状況の面
(`/admin/status`)の先頭とトップの状況の札に知らせが出ます。戻すときは、認証情報を
登録し直し、「接続を試す」→「有効にする」の順に押します(知らせはそこで消えます)。
...
管理画面の状況の面の「AI」の節にある「使用量」（`/admin/status#ai-usage`）と
`GET /v1/ai/usage` で、相手ごとの使用量が見られます。画面の表は**使う相手だけ**に
絞ってあります（使っていない相手も含めた全部は `GET /v1/ai/usage` で）。
内訳・枠の推移・数字の読み方は、表の下の「内訳と枠の推移」に畳んであります。
...
そのうえで管理画面（`/admin/ai`）の claude の行で **登録を「削除」** します。
**削除すると無効になる**ので、**「接続を試す」→「有効にする」の順**に戻してください
（`claude auth status` がコンテナのサインインを見るので通ります）。
...
**claude は「認証情報が無くても有効にできる」相手です**（`CRED_OPTIONAL`）。
required にすると、枠を取るために登録を空にした瞬間、二度と有効にできなくなります。
...
表の下の「内訳と枠の推移」に**内訳**があります（`/admin/status#ai-breakdown`）。**相手 × モデル × 考える量 ×
...
内訳の下に**枠の推移**があります（`/admin/status#ai-quota-trail`）。**控えは「いまどうか」
...
**同じ名前の窓が 2 つ並ぶことがあります。** 名前は窓の長さから作るので、実測で
codex が返す 2 つの 7 日の窓は見分けが付きませんでした（別勘定で、値もまったく違います）。
ぶつかったものは**全部、窓の id を添えて呼び分けます**（`直近 7 日(secondary)` /
`直近 7 日(primary-10080m)`）——片方だけ素のままにすると、どちらが素のままかが
相手の並べ方次第になります。

**相手が返さなくなった窓は下へ回します。** 窓の名前が変わると、その名前の線はそこで
止まります——**消さずに「いまは返ってこない窓」と書いて末尾へ**。消すと後から
「あの数字は何だったのか」を確かめようがなくなりますが、生きている線と同じ顔で
並んでいると読み違えます（**混ざっていた頃の線が「上がったぶん」を大きく持っていると、
直したあとも一番上に居座ります**——実際にそう見えました）。
...
収集の面に**ワーカー**があります（`/admin/collect#ai-workers`）。**巡回を回す相手の順番**で、
網羅の収集のように端から端まで精査し続けるものに使います——**1 つの相手の枠で回し切れる
とは限らない**ので、詰まったところで止まるのではなく次の相手へ振り替えて回り続けるための
並びです。

**先頭から見て、枠に余裕のある最初の相手に頼みます。** 使用率が **80%** を超えている
相手は避けます（`CHIEZO_WORKER_QUOTA_LIMIT` で変えられます）。見るのは上の表と同じ控えで、
**選ぶたびに相手へ問い合わせません**。

**枠を出さない相手は締め出しません。** まだ一度も取れていない相手も同じ扱いです——
「分からない」を理由に外すと、いちばん頼りたい相手が使えなくなります。

**同じ相手でも、枠が別なら別に数えます。** Antigravity は Gemini と Claude/GPT で
週も 5 時間も別勘定で、どちらを食うかは**選んだモデルで決まります** ——
`gemini-3.8-flash-medium` は Gemini の枠、`claude-opus-4-6-thinking` は
Claude/GPT の枠です。まとめて「いちばん詰まっている窓」で見ると、片方が詰まった
だけで相手ごと避けることになり、**逃げ先として置いた段まで巻き添えで飛ばされます**
（実測で、Claude 枠が 100% になった回に、5 時間 78%・週 46% 空いていた Gemini の段が
使われず次の相手へ落ちました）。段の 1 つ目と 2 つ目を同じ相手の別の枠にしておくと、
片方が閉まってももう片方で回り続けます。

**枠を分けられるのは、相手が枠の呼び名を名乗るときだけです。** 名乗らない相手・
段にモデルを書いていないとき・相手が呼び名を変えたときは、今までどおり全部の窓で
見ます（避けすぎることはあっても、使いすぎることはありません）。

**控えの数字が遅れて届く相手がいます。** Antigravity は重いモデルを数本走らせても
申告が動かず、あとでまとめて跳ねることがあります（実測で、2 本終わった後の採取が
まだ 0% で、3 本目の後に 100% へ一度に跳びました）。**こちらの採取の間隔ではなく
相手の申告の遅れ**なので細かく採っても直りません —— 80% の線が効くのは、
じわじわ動く窓（Claude Code CLI・Codex CLI）のほうだと思ってください。

**どれも詰まっていたら、その回は走らせません。** 予定も進めないので、窓が明けば次の周で
走ります。**巡回に書いてある相手へは落ちません**——落ちると、避けたかった相手に頼むことが
あります。

使い方は 2 段階です。

1. この節でワーカーを作る（名前と、相手の並び。モデルも段ごとに決められます）
2. 収集の面(`/admin/collect/<名前>`)の巡回の設定で**どのワーカーを使うか**を選ぶ

ワーカーは 1 つ 1 行の表に並び、段の並び・詰まり具合・次に起きる時刻・待っている数が
開かずに読めます。直すときは名前を押すと、そのワーカーの面(`/admin/workers/<id>`)へ移ります。

**巡回ごとに違うワーカーを選べます。** 「ざっと」は速い相手の並び、「精査」は賢い相手の
並び、というように、回によって欲しい相手が違うためです。ワーカーを選ばなければ、
今までどおり巡回に書いてある相手へ頼みます。

**ワーカーごとに止められます。** 表の行か、ワーカーの面の頭の「止める」を押すと、
そのワーカーからは何も流れなくなります(定時の回も、手で起こす回も、名指しの実行も)。
枠が細いときに、回復するまで止めておくためのものです。待ち行列はそのまま残るので、
「動かす」を押すと続きから流れます。止めているワーカーは名前に「⏸ 止めている」と出て、
状況の面(`/admin/status`)にも 1 行出ます。

**名前が見つからないときは、巡回に書いてある相手で走ります**（ログには残します）。
綴りを間違えただけで無人の層が止まるより、走って控えに相手が残るほうがよいためです。

**振り替えるのは収集だけです。** pta の売買提案は相手を振り替えません——あちらは
「同じ材料で誰が当てるか」を測る場で、書いた相手が変わると成績の意味が変わります。
...
**CLI が動いている相手は飛ばします。** 枠を聞くのはその相手の CLI を**もう 1 本起こす**
動作で、ブリッジは 1 本ずつしか動かしません（認証情報が回る相手では、2 本同時に走ると
権限ごと失効しうるため）。走っている最中に聞くと空くまで待たされて時間切れになるので、
行ごとのボタンは**押せない状態**にし、まとめて取り直すときも**その相手だけ飛ばします**
（失敗としては数えません。相手が落ちているわけではなく、終われば押せるため）。
飛ばしたことは画面にも `GET /v1/ai/usage?refresh=1` の応答（`skipped`）にも出ます。

**押したボタンは「取り直しています…」に変わります。** CLI に聞く相手は往復に数十秒
かかることがあり、何も変わらないと押せたのか壊れているのか分からず、もう一度押されます
——2 本目は枠を食うだけで、相手によっては失敗します。
...
**一覧にはワーカーも混ざります**(`kind` が `worker`)。**収集を外から頼むアプリ向け**で、
巡回の `backend` にその `id`(`worker:<名前>`)をそのまま入れて送れます —— 混ぜないと
「ワーカーに頼む巡回」は Chiezo の画面からしか作れない機能になります。
**ワーカーはモデルも考える量も持ちません**(`models` も `efforts` も空)。渡す先は
そのときの枠で決まるので、ここで 1 つ選んでも**どの相手に対する指定なのかが
決まりません**（段ごとの指定はワーカーの側が持っています）。
**`/v1/ai/complete` の相手にはなりません** —— あちらは 1 往復をその場で投げる口で、
振り替えの判断が入る余地がありません。

**`requested_by` で名乗れます**(管理画面の状況の面の「依頼の履歴」の依頼元に出る)。
...
| `CHIEZO_AI_TRANSCRIPT_DAYS` | `14` | AI に渡したものと返ってきたものを控える日数。**`0` で控えない**。相手の生の出力（CLI の stdout/stderr）も残すので、シェルを渡している相手が何をしたかを後から読める。管理画面では状況の面の「依頼の履歴」の表の**同じ行**から開ける（控えが先に消えた行は目方だけになる） |
...
| `CHIEZO_BRIDGE_EFFORTS` | （下記） | 名乗る「考える量」の候補（カンマ区切り）。**畳んだモデル名を組むのに使います**（画面に欄はありません） |
...
| `CHIEZO_BRIDGE_TIMEOUT` | `3600` | 1 回の上限（秒）。CLI は道具を何度も引くので推論サーバより長い。chiezo-app が待つ `CHIEZO_ANSWER_TIMEOUT`（既定 3660）より短くしておく（待つ側が先に切れると理由が見えない）。**打ち切っても枠は返らない**ので長めに取ってある —— 暴走したものは時間ではなく「止める」で落とす（`POST /v1/ai/inflight/{id}/stop`）。Antigravity にはこの値を `--print-timeout` としても渡す（agy 自前の待ち時間は既定 5 分で、渡さないとそちらが先に切れる） |
...
**考える量の欄はもうありません。モデルの名前に畳んであります**（`sonnet-high` /
`gpt-6-astra-max`）。**どの相手も畳むか slug に埋めるかになった**ので、
会話画面にも巡回の設定にも欄は出ません（`providers.selectable_efforts` が
どの相手でも空を返します）—— **選択肢が 1 つも無い欄は「設定したつもり」だけを
残します**。Antigravity の slug が元からその形で、claude と codex だけ欄が
2 つに分かれていました —— 頼む側は**相手ごとに欄の数が変わる**ことになり、
`--model gemini-3.8-flash-high --effort low` のような**食い違う組み合わせ**も作れました
（どちらが勝つかは CLI 次第でこちらからは読めません）。**選んだつもりが効いていないのは、
欄が無いより悪い**ので、選ばせるのをやめました。

**素の名前も残します**（`sonnet` と `sonnet-high` が並びます）。畳んだ名前しか出さないと
「考える量は CLI の既定でよい」を選べなくなります —— どれか 1 つを選ばされるのは、
任せるのとは別のことです。

**Codex ではこれが効きます。** 段階はモデルごとに違う（最上位のモデルだけが `max` を
受けるなど）ので、畳めば**そのモデルが受けない段階は並びません**。

**出さないことと、受け取らないことは別です。** 既に保存されている設定からは
「モデル + 考える量」の 2 つ組で飛んでくるので、**ブリッジはどちらの形も受けます**。
Chiezo 側も、**素の名前を選んだ回には今までどおり考える量を送ります** —— 相手ごとの
真偽値で決めると、素のほうを選んだ回まで落ちて、**指定した覚えのあるものが指定なしで
走り出します**（画面には何も出ません）。**名前に埋まっているほうが勝ちます**
（そちらのほうが具体的で、選び直した結果だからです）。

**終わった行にも考える量が出ます。** 畳んで送る相手では 2 つ組を送らないので、
選んだ値はどこにも残らず、返ってくるのは相手が解決した素の名前（`claude-fable-5-1`）
—— **走っている間だけ見えて、完了すると消えていました**（Antigravity だけ残っていたのは、
あちらの slug に元から段が入っているからです）。いまは**送ったモデルの名前から
取り出して控えます**ので、「依頼の履歴」の表でも後から読めます。
...
### 考える量（エフォート）はモデルの名前に入っています

**選ぶ欄はどこにもありません。** 段階はモデルの名前の一部で、モデルを選べば決まります。

段階そのものは CLI ごとに違います（畳んだ名前の後ろに付くのがこれです）。
...
名前に畳むときは控えから 1 件ずつ引くので、**そのモデルが受ける段階だけ**が並びます。
1 本の一覧として名乗るとき（検証に使う側）は、**どのモデルと組み合わせても通る段階だけ**に
絞ります —— union にすると、通らない組み合わせを受け付けてしまいます。
...
| `CHIEZO_ANSWER_TIMEOUT` | 相手による | 相手を待つ時間(秒)。DB クエリの 5 秒とは別枠。**CLI ブリッジ経由は 3660、API で直に叩く相手と推論サーバは 120**。ブリッジは道具を何度も引くので桁が違ううえ、ブリッジ自身の上限(`CHIEZO_BRIDGE_TIMEOUT`。既定 3600)より**待つ側が長くないと**、向こうの判断が見えないまま ReadTimeout になる |
