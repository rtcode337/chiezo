...
- **消したものは墓場(`graves`)に残り、足す回はそこにあるものを連れ戻さない。**
  消す回と足す回は別々に走るので、残さないと戻ってくる —— 足すほうは「いま名簿に
  いるか」しか見られず、なぜ居ないのか(一度も入っていないのか、調べたうえで外したのか)
  までは分からない。**`PATCH` で空の配列を渡せば空にできる**(消し間違いの逃げ道)
