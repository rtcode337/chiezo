...
`CHIEZO_COLLECT_DIR`(溜め先)と `CHIEZO_NOTES_DIR`(定義の置き場)の両方が要る。

**溜まる先は長期記憶**(`corpus/`)。焼くのは ingest で、契約は固化と同じ ——
集めたものは待ち行列に置かれ、管理画面の「長期記憶へ焼く」(普通の取り込み)で
`corpus/` に入る。焼いた後は `/v1/news/search` のように普通のソースとして引ける。

**毎回焼き直すが中身は積み上がる。** 素材が「前世代 + 新しく集めたぶん」なので、
ブルーグリーンに乗せたまま追記として振る舞う(世代は今と 1 つ前だけ残る)。
同じ見出しが再び来たら飛ばし、焼くときは前世代の同じ見出しを置き換える。
...
# 一覧(間隔・次にいつ走るか・溜まった件数が入る)
curl -s "$BASE/v1/collect" | jq .

# 予定を待たずに1回走らせる(有効にしてある収集だけ。止まっていれば403)
...
**失敗しても次回の予定は入る**ので、一時的に相手が落ちていても回り続ける。

**管理画面(`/admin` の「集める」)からも足せる**。一覧には間隔・次にいつ・溜まった数・
前回の結果が並び、行ごとに「有効にする / 止める」「いま走らせる」「削除」がある。
