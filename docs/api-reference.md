...
人と AI が読み書きする場所なので、混ぜると**人が消せてしまう**・
**1 件に収める都合で中身に上限が要る**(消したものの控えが 2,000 件で頭打ちになり、
溢れると古いものから静かに戻ってきた)。

**置いてあるものは管理画面から読める**(記憶の面の「設定の置き場」)。人が触らない
置き場だが、見えないままだと「設定がどこに、いつの姿で残っているか」を確かめる
手立てが無い —— 収集が消えた・戻ってきたのような話を追うときに要る。
**読むだけ**で、直すのはそれぞれの画面から(収集なら「集める」の各収集の面)。
...
**同じ軸を手掛かりの確かな順に並べるときは `"fallback": true` を付ける。** 前の規則が
同じ接頭辞のタグ(上の例なら `年代:`)を作っていれば、その規則は作らない ——
付けないと当たった全部が足され、生年と没年の両方を持つ人に年代のタグが 2 本付く。

**捕まえた値を別の書き方へ読み替えるときは、`pattern` に `map` の表を付ける。**
`{"pattern": "^(\\d{1,2})世紀.+の画家$", "format": "年代:{1}", "map": {"18": "1700頃-1800頃"}}`
のように書くと、`{1}` に表で引いた値が入る(表に無い値に当たったら作らない)。
値ごとに規則を並べずに済む(規則は 20 個まで)。
...
- **そのまま載る値だけ**(文字列・数・真偽)。入れ子は通さない ——
  **そのまま載る値の並びは通す**(「一緒に出てくる語」のように、添える事実が
  並びになることがある。並びの中の入れ子は落ちる)

##### 脇書きは重ねる(`extra`)

1 件は焼くたびに丸ごと置き換わるが、**`extra` だけは重ねる**。

- **書いたものだけが変わる。** 触れなかった鍵はそのまま残る —— 機械で運んだ事実は
  AI が手を入れる回には返ってこないので、置き換えると最初の手入れで静かに消える
  (1 件ずつ減るので、消えすぎの歯止めもすり抜ける)
- **`null` は「この鍵を消して」の印。** 書かなければ残る作りなので、間違って入った値を
  落とす口がここに要る
- **足すだけの回(`only_new`)は、まだ無い鍵だけを埋める。** 上書きも削除もしない ——
  後から指定に鍵を足しても、既にあるものには届かないままになるため。
  **ただし機械で引く回(`use_extract`)は、運んできた鍵を入れ替える** —— 数えた値は
  回るたびに変わるので、入れ替えないと最初に拾った日の数が残り続ける
- 数には天井がある(消さない作りなので、置かないと回を重ねるだけ増える)
...
##### 文書ではなく、タグを名簿にする(`of: "tags"`)

`extract` は既定で**そのタグが付いた文書**を 1 件ずつ名簿にするが、`of: "tags"` と書くと
**そのソースのタグを 1 語ずつ**名簿にする。**溜めたものの索引を作る**ための口で、
「集めた記事の話題の網を育てる」のような収集がこれに当たる —— 語を AI に思いつかせると、
記事に出てこない語が混ざるうえ、同じ語が回ごとに違う表記で増える。

```json
{
  "source": "tech_news",
  "of": "tags",
  "skip": ["^_chiezo_", "^重要度:", "^(ニュース|記事|論文)$"],
  "min_docs": 2,
  "recent_days": 7,
  "links": 6,
  "tags": [{"const": "トピック"}]
...
1 語ごとに、数えた事実が `extra` に載る。

| 鍵 | 中身 |
| --- | --- |
| `docs` | その語が付いている文書の数 |
| `docs_recent` | `recent_days` の窓に入るぶん。**時間軸はここ** —— 合計だけだと、昔よく出てきた語がいつまでも大きい |
| `last_seen` | 最後にその語が付いた日 |
| `links` | 一緒に出てくる語(`語:回数`。上位 `links` 件) |

- **日付は配信日があればそちら**(`extra.published_at`)。集めた日で数えると、古いものを
  まとめて取り込んだ日に、その語が急に動き出したように見える
- **読者に出さない印の付いた文書は数えない**(消えたもの・まだ精査していないもの)。
  そこから語を起こすと、外したはずの宣伝が話題として並ぶ
- **外す語は頼む側が書く**(`skip` は正規表現)。どれが分野で、どれが媒体名かは頼む側の
  アプリが決めていて、こちらには知る手立てが無い
- **`min_docs` は「何件以上付いていれば拾うか」**。1 件しか付いていない語は、まだ話題か
  どうかも分からない(次の回に増えていれば入る)
- **切るのは `skip` を落としてから**(先に切ると、外す語が上位を埋めているぶん拾えない)
- **`use_extract: true` の巡回と組にする。** 数は回るたびに変わるので、
  拾い直す回が要る —— **足すだけの回でも、機械が運んだ鍵は入れ替わる**
  (本文とタグは AI のものなので触らない)

#### 見出しだけを差し込む(`{names}`)

`{current}` は 1 件ごとに本文を 200 字まで見せるので、**育った収集ほど 1 件が重くなり、
数百件で天井に当たる**(`MAX_MATERIAL_CHARS`)。しかも**切れるのは新しいほう**
(並びは古い順)—— 重なりを畳む・親子を決めるような回では、いちばん見てほしい
「入ったばかりのもの」が落ちる。

`{names}` は**見出しとタグだけ**を差し込む。1 件がおよそ 1/6 になるので、数千件でも入る。

```
いま持っているものの見出し(全 214 件):
- AI 【トピック/分野:AI】
- AIエージェント 【トピック/分野:AI/親:AI】
- MCP 【トピック/分野:AI】
```

- 使いどころは「**何を持っているか**」だけが要る回 —— 畳む、親子を決める、
  同じものを二度足さない。本文の良し悪しを見る回は `{current}` のまま
- **消えたものは出さない**(ここは「いま何を持っているか」を見せる場所)
- **消す力は持てる**(`{current}` / `{recent}` と同じ。消す相手を見出しで指せる)
- **目を通した印は付かない**(`_chiezo_unreviewed` は外れない)—— 見出しだけでは
  読んだことにならない

#### まだ目を通していないものだけを差し込む(`{unreviewed}`)

機械で入ったもの(フィードで引く回・機械で引く回)には「まだ AI が目を通していない」印
(`_chiezo_unreviewed`)が付く。`{unreviewed}` は**その印の付いたものだけ**を、
`{current}` と同じ形(見出し・タグ・本文の頭・出典)で差し込む。

- **渡したものだけ印が外れる。** 次の回は読み直さない —— 調べ終えたものを
  何度も調べ直さないための口
- **古い順に載せる。** 入り切らなかったぶんは印が残り、次の回に回る(本文にもそう書く)
- **`{recent}` との違い**: あちらは「前回この巡回が走った時刻より後に動いたもの」で、
  時刻で区切る。落ちた回や入り切らなかったぶんは次の窓から外れ、機械が数を
  書き換えるたびに同じものが差分に入り直す
- 区画を持つ収集では、その区画のぶんだけ(`{current}` と同じ)
- **消す力は持てる**(`{current}` / `{recent}` / `{names}` と同じ)
- 全体を見る仕事(重なりを畳む)と組むときは `{names}` を並べて書く
...
- **`tag` で絞れる。** 1 つの収集には種類の違うものが混ざる(記事とまとめ、など)ので、
  絞れないと材料にまとめが混ざる
- **読者に出さない印の付いたものは渡らない**(消えたもの・まだ AI が目を通していないもの)。
  材料に収集を選ぶのは「**そちらの AI が選り分けた後のもの**を読みたい」からで、
  混ぜると、向こうが落とした宣伝や重複からこちらが育つ
...
**配信元が 1 件ごとに付けている語も渡します**(`<category>` / `<dc:subject>` / Atom の
`term`)。行の末尾に `[語: Codex、OCI]` の形で出ます —— 書き手が付けた語は、AI が
推し量った分野より確かなためです(1 件あたり 5 個まで)。**文書のタグには混ざりません。**
...
| `band` | 分類のタグ × 数のタグ。同じ括りの中を、数の軸に沿って人数が揃うところで区切る | `prefix` / `value` |

`by=geo` では **`origin`(`[緯度, 経度]`)を書くと、そこから近い順に広がります**。
書かなければ鍵の文字列順で、矩形の鍵は南西の角から始まるので**日本の地図なら
...
全点をメモリに載せる**ので、素材を流すのと同じ回に乗ると山が二つ重なります ——
...
- **その収集を焼いている最中は 409 で断ります**(あちらも終わりに台帳を書き戻すため)。
  別のソースを焼いている最中なら押せます
- **押した人を待たせません。** 母集団を丸ごと 1 周舐めるので、数十万件では
  ブラウザが先に切れます —— 押すとすぐ画面が戻り、**割り直しは裏で走ります**。
  終わると「N 区画に割り直しました」、落ちれば理由が区画の節に出ます
- **走っているあいだは押せません**(409)。2 本が同じ母集団を読むと、メモリも
  時間も倍になるうえ、後に終わったほうが勝つだけです
...
- **収集(または巡回)を止めると、待ち行列に残っているぶんも流れません。**
  積んだあとに止めたぶんは行列から外れます —— 押した「止める」が効かずに
  1 回ぶんの取り込みが走ってしまうため。管理画面の「今すぐ実行」だけは別で、
  止めてある収集でも走ります(有効にする前に試せる道です)。
...
- **`sweeps` を書かない収集は、定義そのものが 1 本の巡回**(`既定`)として動く。
  今までの収集は何も変えなくてよい
- **`prompt` を書けば、その巡回だけ別の依頼文で走る**(空なら収集のもの)。頼むことが
  巡回ごとに違う(埋める / 見直して消す / 漏れを足す)のに、1 つの文で全部を頼むと、
  どの回も同じ薄さの仕事になる
- **`use_extract: true` の巡回は、AI を呼ばずに `extract` の指定をもう一度走らせる。**
  名簿を最新に保つための回 —— 機械で埋めるのは「進み具合が空の 1 回目だけ」なので、
  そのあと外のカテゴリが増えても入らない。**`only_new` と組にして使う**
  (組にしないと、AI が肉付けしたぶんを名簿の薄い内容で上書きする)
- **`by_hand: true` の巡回は、AI を呼ばずに「束」を書き出す**(手で回す回)。
  web の画面から使う AI(Gemini など)に頼むための道です —— Chiezo からは呼べない
  相手なので、**依頼文をファイルにして人が渡し、答えのファイルを読み込みます**。

  ```bash
  curl -s -X POST "$BASE/v1/collect/tl_lunch/handoff"            # 束を作る
  curl -s "$BASE/v1/collect/tl_lunch/handoff/file" -o handoff.md # 渡すファイル
  curl -s -X POST "$BASE/v1/collect/tl_lunch/handoff/answer" \
       --data-binary @answer.json                                # 答えを読み込む(焼く)
  ```

  - **両端はいつもと同じ部品**です。依頼文は AI に投げるのとまったく同じ文で
    (言い換えると、手で回した回と AI に頼んだ回で違うことを頼むことになります)、
    答えは AI の返事と同じ読み方で拾います(前置きや ``` の囲みが混ざっても通る)。
    そこから先 —— 焼く・墓標・URL の重複・未精査の印外し・区画の印 —— は
    まったく同じ道を通ります
  - **ファイルには「何をして、どの形で返すか」を先に書きます。** 材料だけ渡すと、
    web の画面は要約や感想を返してきます。**添えて送る一言**も画面に出ます
    (ファイルを添付するだけでは読んで作業してもらえないため)
  - **預かれる束は 1 つだけ。** 「作る → 渡す → 読み込む」が一巡するまで次を
    作りません(溜まると、どの束の答えなのかが分からなくなります)
  - **印が付くのは、束に入れて渡した区画だけ**です。答えが返るまでに台帳は動きうる
    ので、焼くときに選び直しません —— 渡した範囲と印を付ける範囲がずれます
  - **次の束は、答えが焼けたら自動で組みます**(区画を回る収集のみ)。
    区画を持たない収集は押したときだけ —— あちらは「いまの外の様子」を聞く回なので、
    聞きたいときに作るほうが自然です
  - **時計では走りません。** 走らせる中身(人が持ち帰った答え)が揃うのは
    読み込んだときなので、予定で起こすと空振りします
- **消したものは消さずに、消えた印(`_chiezo_removed`)を付けて残す。**
  消していた頃は、消す回と足す回が別々に走るせいで外したものが次の回で戻ってきた ——
  足すほうは「いま名簿にいるか」しか見られず、なぜ居ないのか(一度も入っていないのか、
  調べたうえで外したのか)までは分からない。見出しの控えを定義に持って止めていたが、
  あれは 1 件に収める都合で上限が要り、溢れると古いものから静かに戻っていた。
  **残せば、その問題ごと消える** —— 見出しは既にいるので足す回は素通りし、上限も要らない。
  **読み口は既定でこの印の付いた文書を返さない**ので人には出ず、集める層は焼いた DB を
  直に読むので **AI からは見えたまま**(何を外したかを自分で確かめられる)。
  **本文はそのまま残る** —— 印を外せば元の中身のまま戻る。消した理由は本文を
  置き換えずに脇書き(`extra.removed_reason` / `removed_at`)へ置く ——
  置き換えていた頃は、消し間違いを戻しても「なぜ消したか」の 1 行しか残っておらず、
  育てたぶんを集め直すことになっていた
- **消しすぎの歯止めは「印を付けた件数」で数える**(`keep_ratio`)。件数そのものは
  減らないので、結果の件数で見ていると 9 割に印が付いた回が素通りする
- **区画の中身は AI が自分で引ける**(`GET /v1/collect/{name}/partition?key=…`、
  MCP の `collect_partition`)。差し込み(`{current}`)でしか見えなかった頃は、
  **1 回の依頼に載る量が上限**だった —— 入り切らないぶんは黙って落ちるので、
  「この範囲の全員」を前提にした漏れ探しが成り立たなかった。
  **この口は消えたものも返す**(何を外したのかが分からないと、同じものをもう一度
  挙げることになる)。読み口の既定と違うのは、ここが集める層のための口だから。
  **画面からも同じところを見られる** —— 管理画面の台帳で区画の名前を押すと、
  その区画に入っているものが並ぶ(`/admin/collect/<収集名>/partition?key=…`)。
  **どの巡回がいつ見終えたかも出ます**(日本時間)。一周に何十日もかかる台帳では
  「見た」より「いつ見た」のほうが知りたい —— 古い順に配られるので、次にどこが
  回ってくるかもそこから読めます。
  **鍵から範囲を作って SQL で先に絞ります。** 絞らずに全文書を読んでいた頃は、
  本番の食事処(686,602 件)で**いちばん小さい区画を開くのに 48.9 秒**かかりました
  —— 中身の件数は関係なく、全部が「全表を本文ごと読む」ぶんです。
  **絞りは速さのためだけで、正しさは今までどおりの所属判定が持ちます**ので、
  返るものは変わりません。矩形(`doc_coords`)とタグ(`doc_tags`)は厳密に絞れ、
  見出しは**隣の区画の始まり**で切ります(鍵の終わりではありません —— 区画は
  次の始まりの手前までを引き受けるので、そこで切ると隙間の見出しを落とします)。
  **帯は絞りません**(分類の値は鍵に入るときに区切りの字を落としてあり、
  鍵からもとのタグを組み直せないため)
- **細かく切れすぎた区画は隣とまとめる。** 割り直しの引き金は「育った」と
  「空になった」しかないので、中身が別の区画へ移って痩せた帯は痩せたまま回り続ける
  —— 数人のために 1 回ぶんの枠を使うことになる。まとめるのは**周回の記録が同じ
  隣どうし**で、**合わせても `target` の 8 割に満たないとき**だけ。
  **記録は、どの巡回で見終えたかだけを比べます**(時刻は比べません。まとめた区画の
  時刻は巡回ごとに古いほうになります)。
  **まとめられるのは「幅」で表した区画だけ**(年代の帯・見出しの範囲・矩形)——
  隣り合うぶんを覆う 1 つの幅があるので、鍵の読み方も、どの文書がどこへ入るかの
  規則も変わらない
- **範囲で区切っていない小さい分類は、値を並べた 1 区画に寄せ集める**
  (`アルメニア|エストニア|-`)。隣とまとめる道は分類をまたげない(2 つの名前は
  1 つの名前で表せない)ので、1 人しかいない国はその 1 人で 1 区画のまま残っていました
  —— その 1 人のために巡回の 1 回ぶんの枠を使うことになります。寄せるのは
  **数の軸で 1 つも割れていない**(その分類ぜんぶで 1 帯)・**周回の記録が同じ**もの。
  **大きさでは絞りません** —— 年が全部で 1 帯なら受け持ちは「その分類の全員」なので、
  入るなら寄せてよい(「5 件以下だけ」にしていた頃は、1 帯のままの国がいくつも
  独りで残っていました)。**引き受けている分類は鍵に全部並べて持ちます** ——
  どこまでがその区画かが分からなければ、漏れを問えないためです。
  数が `target` の 8 割に届く前でも、並べる値が 20 を超えたら止めます
- **`only_new: true` の巡回は足すだけ。** 既にある見出しが返ってきても触らず、
  墓標も読まない。「この範囲/括りから漏れているものを足して」と頼む回に要る印で、
  **AI の判断に頼らずに Chiezo 側で保証する** —— 見せられるのはその区画のぶんだけなので、
  AI には「もう居るかどうか」が分からない(別の括りに入っていることも、タグが
  間違っていることもある)。触らせると、既にいるものが薄い内容で上書きされ、
  持っていたタグごと落ちる
- **`on_demand: true` の巡回は時計を持たない。** 定時には走らず、割り込みで
  頼まれたときだけ動く —— 狙いは**頼む相手を定時のものと別に決めておく**こと。
  割り込みは人が待っている場面なので速い相手に頼みたい / 1 件をじっくり
  調べさせたい、のどちらもあり、定時の巡回の設定を流用すると
  「どちらの都合で選んだ相手か」が言えなくなる
...
#### 選んだ区画だけ 1 回走らせる(試し撃ち)

**枠が細いときに「直したコードを 1 区画だけ本番の形で試す」ための道。**
管理画面の区画の面(`/admin/collect/<収集名>/partition?key=…`)に
「この区画だけ 1 回走らせる」として出ています。

- **ふつうの回として走ります** —— 区画に「見た」印が付き、進み具合も次回の予定も
  進みます。**割り込みと分かれるのはここ 1 点**(あちらは定時の巡回に影響を出しません)
- **見るのは選んだ区画だけ**なので、**AI の呼び出しは選んだ数だけ**です
  (定時の回は `per_run` 区画ぶん = 食事処なら 5 回)。**一度に選べるのは
  20 区画まで**(`CHIEZO_MAX_TRIAL_PARTITIONS`)—— 枠は区画の数だけ減るので、
  押してから気づいてもそのぶんは戻りません
- **選び方は 2 つ**: 台帳の表で行の頭にチェックを入れるか(畳んである残りの区画も
  開いて選べます)、区画の面を開いてその 1 区画だけ走らせるか
- **1 つでも台帳に無ければ走りません**(409)。通ったぶんだけ走ると、押した人には
  全部を見たように見えて、抜けた区画だけが黙って飛ばされます
- **焼きます。** ドライランは焼かないので、**焼く側で落ちる回を再現しません**
- **頼む相手・モデル・ワーカーを選べます。** 空いている相手へ回したいときは
  **ワーカー**が効きます(枠を見て選ぶのはあちらの仕事)。考える量の欄はありません
  —— モデルの名前に畳んであるので、モデルを選べば決まります
- **上書きは 1 回きりで、保存しません。** 巡回の設定は書き換わらないので、
  次の定時の回は今までどおりの相手で走ります —— 設定を直して試すと、直したまま
  定時の回が走り出します
- **相手を替えるとモデルは引き継ぎません**(相手が変われば通る名前も違う。
  `sonnet` は codex にありません)
- **止めてある収集でも走らせます**(「今すぐ実行」と同じ。自動実行を止めたうえで
  直したものを試す、がまさにこの口の用途です)
- **区画の割り直しは走りません。** 割り直しは母集団を丸ごと 1 周舐めるので、
  **1 区画だけ試すための回でいちばん重い処理が走る**ことになりますし、
  **割り直すと名指しした鍵が消えることがあります**(そうなるとどの文書も一致せず、
  AI は「誰も居ない」と読んで空振りします)。割り直したいときは、先に
  「区画を割り直す」を押してください
- **台帳に無い鍵は 409 で断ります。** 開きっぱなしの画面から押すと起きるので、
  区画の面から選び直してください
- **機械で引く回・外の道具で引く回は選べません**(400)。あれは指定を 1 本引いて
  全部を返す回で**区画を見ない**ので、名指ししても全件の回が走ります
...
- **台帳に無い区画は 409。** 区画は割り直すと鍵が変わるので、少し前に読んだ鍵でも
  起きる。受け付けたあとに消えた場合は、AI を呼ばずに失敗として控える ——
  走らせると差し込みが空のまま AI を呼び、返りが既存の中身を見ないまま置き換える
- **範囲の外の既存の 1 件は置き換えない。** 区画で絞った直す回(割り込みも定時の回も)で、
  差し込んだ範囲に居ない既存の見出しが返ってきたら、そのまま残して控えに書く。
  居ない見出し(漏れとして足すもの)は通る
...
- **`prompt` を渡すと、その回だけその依頼文で走る。** 差し込み口(`{cursor}`
  `{current}` `{partition}` `{feed}`)はいつもどおり効く。**保存はしない** ——
...
# **保存しない** —— 定時の回は元の依頼文のまま走る。AI には「確かめてから反映して」
# 「この回の範囲の外には触らないで」と添える。note は全部の区画に同じものが載る。
# 区画ごとに分けるなら notes({"区画の鍵": "その区画のぶん"})—— その区画を聞く回に
# だけ載る(書いた区画は名指ししたことになる)。1 回で何区画も見るときはこちらを使う
...
なっていれば一周の見込みが 4 倍ずれます。**測るのは集めるところまで**で、焼くぶんは
...
署名が付くのは **AI が返した 1 件だけ**で、機械で引いた回(`extract`)と
...
**設定と一緒に見本も引ける**(`GET /v1/collect/{name}?samples=20`)。こちらも
...
直して回ることになり、しかも漏れるのは読者の画面だけ**なので気づくのが遅くなります。
...
**地図辞典から国ぶんの名簿を作るときは、この天井に当たる。** 1 件のバイト数は運ぶ
項目で倍以上違い、見出しと本文だけなら 256 バイト、住所・電話・サイト・座標まで運ぶと
474 バイト(どちらも実測)。60 万件の名簿は 280 MB を超えるので、既定のままでは断られる
—— 素材は 1 本の文字列で組むため、上げたぶんはそのままメモリに載ることを見込んで決める。

時計は Chiezo が持つ(`CHIEZO_COLLECT_TICK_SECONDS`、既定 60 秒で見に来る)。
予定が来た回は**取り込みの待ち行列に並び**、走らせられる本数(chiezo-trigger の
`CHIEZO_INGEST_SLOTS`、既定 1)に空きが出た周で先頭から流れる。予定は流せたときに
進むので、待っているあいだに飛ばされることは無い。待ち行列は管理画面の状況の面に出る。
**失敗しても次回の予定は入る**ので、一時的に相手が落ちていても回り続ける。

**`POST /v1/collect/{name}/run` も、埋まっていれば並ぶ**(202 と
`{"queued": true, "position": 何番目}`)。空いていればその場で起こして 200。
`GET /v1/ingest/status` は `running`(何か走っているか)に加えて、`full`(空きが無いか)・
`slots`・`jobs`・`waiting`(待っている数)を返す。**割り込み(`…/focus`)は並ばない** ——
埋まっていれば 409 で断る。
...
**巡回の `backend` にはワーカーも書ける**（`worker:<id>`。`GET /v1/ai/backends` が
`kind: "worker"` として返す値をそのまま入れる）。**結ぶのは id で、名前ではありません**
—— 名前は画面に出す札で、直しても名指しは動きません（`label` に出ます）。**その巡回の `model` と `effort` は
...
読めない。**時計を持たない巡回(割り込み用)には出ない**(口のほうでも断る)。
**その回がワーカーの待ち行列に積まれていたら、外します** ——
「今すぐ実行」も外のアプリからの依頼(`POST /v1/collect/{name}/run`)も行列を
通さずその場で走らせるので、残すとそのワーカーが起きたときに同じ回がもう一度
流れます(枠を 1 回ぶん余計に食う)。**全部のワーカーを見ます** ——
積んだ後に巡回のワーカーを付け替えていれば、積まれているのは前のワーカーの
行列のほうだからです。**画面の「削除」は設定と焼いたものをまとめて消す**
(元に戻せない)。設定だけ消して中身が残ると、一覧から消えたのに検索には出続けるものが
できるため。**焼いたものを消せないときは、設定も消しません**(409。理由を返します)。
片方だけ消えると、次に集めたときに前の世代として戻ってくるためです。
取り込み(chiezo-trigger)が応える状態にしてから、もう一度押してください。
**まだ 1 度も焼いていない収集は、そのまま消せます**(消す先が無いので)。**REST の `DELETE /v1/collect/{name}` は設定だけ**で、溜まったものは残る。整理(作り直し)の行は名前の横にそう出て、
前回の欄には消えた件数と見出しが並ぶ。
...
**名前はソース名になる**ので 1 つしか持てない。**ハイフンは使えない** —— 世代ファイル名
...
ブラウザの帯を消して開くので、**戻る・読み直す・進む**を画面の下に自前で出します
—— 帯が消えたぶんを補うためで、ブラウザで開いているときは出ません。
**下に引っ張ると読み直します**(いちばん上にいるときだけ。入力の中では掴みません)。
**Service Worker は持ちません** —— 抱え込むと、更新しても古い版が出続けるためです
(オフラインでは動きません)。

**面に分かれています。** 見に来る目的が違うものを 1 枚に積み上げると、いま見たい節に
着くまで無関係な表を何度もスクロールすることになるためです。`/admin` はトップで、
各面への入口(概況を 1 行ずつ添えた札)だけを出します。どの面からも、見出しの
「Chiezo 管理画面」を押せばトップへ戻れます。

| 面 | 何があるか |
|---|---|
| `/admin/status` | 状況。ディスクの空き、取り込み、収集、AI(走っている依頼・使用量・依頼の履歴) |
...
どの面にも同じ帯を出すので、トップへ戻らずに行き来できます。**どの画面にもフッタがあり、
いま動いているビルド**(ビルド日時と元のコミット)が出ます —— 様子がおかしいときに、
古いイメージのままかどうかをその場で確かめられます。

状況の面に出るのは、**ディスクの空き**(取り込みがいちばんディスクを食うため)と、
**取り込み**・**収集**・**AI** の 3 つの節です。AI の節には、**いま走っている依頼**、
**使用量**、**依頼の履歴**(既定では閉じていて、ページを送ったときと失敗だけに
絞ったときだけ開きます)が並びます。
取り込みの節には、**取り込みの実行履歴**(終わった回を新しい順に 20 本まで。chiezo-trigger の
メモリーにだけあるので、コンテナを作り直すと消えます)を、そのすぐ下の「収集」の節には
**いま走っている収集**と**全部の収集の実行履歴**を置いてあります。2 つの実行履歴は既定では閉じています
(収集のほうは回で絞ったときとページを送ったときだけ開きます。1 ページ 30 件で、
古いほうへ送れます)。1 つの収集のぶんは、その収集の面にあります。
走っている依頼が無ければ何も出しません(空の表を置くと、動いていないことのほうが目立つため)。
**取り込みの様子はこの面にしか出しません**(記憶の面には、長期記憶の表の上に
「chiezo-trigger が立ち上がっていない場合、再構築と削除はできません」とだけ書いてあります。
OSM・Wikipedia の初期化の面は、取り込み側に繋がらないときだけ断り書きを出します)。**走らせるボタンを押すと、この面へ移ります** —— 初期化・
再構築・収集の「今すぐ実行」・やり直し・区画を選んで走らせる・割り込み・取り込みを止める、
のどれも、押した人が次に見たいのは進み具合だからです。

使用量の表は**使う相手だけ**に絞ってあります(全部の相手は `GET /v1/ai/usage` で)。
**窓は相手が返したぶんを全部並べます** —— いちばん詰まっている窓 1 つに畳むと、
5 時間の窓しか見えない相手が出るためです(短い窓が詰まっていても週の窓が空いていれば、
重い仕事は頼めます)。表の下の「内訳と枠の推移」に、誰が枠を食ったのか・いつ跳ねたのかと
数字の読み方を畳んであります。取り直すボタンは、まとめても 1 件ずつでも状況の面へ戻ります。
...
並びます。登録済みソース(文書数・DB の大きさ・dump_date・構築日時・
...
DB の大きさ(`size`)はいま配っている世代のファイルのもので、1 つ前の世代は含みません
(切り替えのあと 1 つ前も残るので、ディスクはおよそその倍を食っています)。

その下に「長期記憶に何を入れるか」の操作があります。

- **未初期化データの初期化** — `chiezo-trigger` 側の既知ソース一覧に載っているが `/data` に
  まだ `.db` が無いもの向けの「初期化」ボタン。**長期記憶の節の中に畳んである**
  (開くのは新しいソースを入れるときだけなので、日々見る表の間に挟まない)
...
**使用量**(`/admin/status#ai-usage`)には、相手ごとに**枠の残り**(聞ける相手だけ)と
**Chiezo が使ったぶん**が並びます。枠は開いたときには聞きに行かず、控えてある値と
取得時刻を出します(取り直しは行のボタン)。同じ内容は `GET /v1/ai/usage` でも
取れます —— 詳しくは [AI と話す](ai.md#使用量を見る枠の残りとchiezo-が使ったぶん)。

状況の面の**依頼の履歴**(`/admin/status#ai-history`)には、会話・絵・音・動画・声のどれでも、
...
応答が、先頭だけ畳んで出ます（続きは「全文を開く」）。**残る長さが別**なので、
...
**走っているものは止められます**(行の「止める」/ `POST /v1/ai/inflight/{id}/stop`。
id は `GET /v1/ai/inflight` の `calls[].id`)。1 回の上限は 60 分まで伸ばしてあるので
—— 途中で打ち切っても枠は返らず、集めたぶんごと消えるだけなので「走っているなら待つ」に
倒しました —— 暴走したものを落とす手段がここになります。**押しても即座に消えるとは
限りません**: 実際に手を離すのはその往復を掴んでいるワーカーで、数秒おきに合図を
見に来ます(そのあいだ行は「止めています」と出ます)。手が離れると繋ぎが切れ、
CLI ブリッジはそれに気づいて CLI を殺します。生成(絵・音・動画・文章)を止めるのは
`POST /v1/media/jobs/{job_id}/cancel` のほうです。
...
**3 つのイメージを並べて出します**(chiezo-app / 取り込み / 立っている CLI ブリッジ)。
別々に焼かれるので**片方だけ古いまま**が普通に起き、そのまま「直したはずの不具合」を
追いかけ続けることになります。**立っていないものは出ません** —— 取り込みは
`GET /status` の `build`、ブリッジは `GET /health` の `build` / `built_at` から読みます
(ブリッジは LAN に口を開けないので、版を外から確かめる手段がここだけです)。
...
ファイル名(`source.png`、`source-2.png`、…)が決まります。**役割の名前は持たない**ので、
「1 枚目は姿勢の見本、2 枚目は絵柄の見本」のように依頼文の中で書いてください ——
種類を増やすより、そのほうが後から効きます。**直すほう(`edit`)は 1 枚だけ**です
(何枚も同時に直すという指示が成立しません)。画面には渡した順に番号つきで並びます。

**古い組へ遡れます。** 一覧は新しい順で、`?page=2` で前の頁へ進みます
(`GET /v1/media/groups` は `offset`)。**置き場の掃除とは別の話**で、
...
聞きに行く**のは依頼として成立しないためです。知識を引く口はそのまま使えます。
...
**画面からも頼めます。** 「作ってもらう」の欄に依頼文と相手を入れて押すと job が立ち、出来たものが同じ画面に組として並びます。**会話できない相手（自前の GPU など）に頼む道はここだけ**です —— 会話の相手に作らせる道もありますが、そちらは話せる相手に限られます。

課金の走る操作を画面に置いてよいのは、管理画面が **Chiezo を操作している人だけが開ける**前提だからです
(会話も収集の即時実行も、押せば枠を使います)。
...
| `requested_by` | 誰が持ち込んだかの名乗り。画面に「依頼元」として出ます |
