...
**見本が 1 つ、止めた状態で最初から入っている**(`tech_news`)。プロンプトの書き方 ——
とくに `{cursor}` と `next_cursor` の使い方 —— の手本で、有効にすれば 6 時間ごとに
技術ニュースを集める。要らなければ削除でよい(消したら戻ってこない)。
...
その下は**AI 依頼の失敗**(`/admin#ai-failures`)です。会話・絵・音・動画・声のどれでも、
落ちたものが新しい順に並びます(日時・依頼の種類・相手・状態・理由・依頼文の大きさ)。
**プロンプトと応答は残していません** —— 呼んだ側の材料がそのまま入るためで、
大きさだけ残してあるのは失敗が大きさに寄っているのかを後から見分けるためです。
**会話と生成を分けていません**: どちらで落ちたか分かっていない人が探せなくなります。
機械で読むなら `GET /v1/ai/failures`。
