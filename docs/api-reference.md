...
#### 集める(append)と整理する(rebuild)

**集め方は 2 つある**(`mode`。既定は `append`)。同じ機械を使うが、
**素材の作り方と、要る守りが変わる**。

| | `append`(集める) | `rebuild`(整理する) |
|---|---|---|
| 素材 | 前世代 + 今回のぶん | **今回のぶんだけ** |
| 返らなかったもの | 残る | **消える** |
| 使うところ | 外から新しいものを取ってきて積む | 既にある内容の分類をやり直す・重複をまとめる・言い回しを揃える |
| プロンプト | `{cursor}` で前回の続きへ進む | **`{current}` に今ある内容が差し込まれる** |
| 歯止め | 要らない(減らないので) | `keep_ratio`(既定 0.5) |

**作り直しのプロンプトには `{current}` が要る**(無いと 400 で断る)。ここへ今ある
内容が差し込まれるので、AI は「何を残すか」を見たうえで全体を答えられる。
入れないまま作り直させると、AI は今の内容を知らないまま「全体」を答えることになり、
返らなかったものが全部消える。

**減りすぎたら焼く前に止まる**(`keep_ratio`)。前世代の何割を下回ったら断るかの値で、
既定は 0.5。AI が変な日に当たった 1 回で、育てた分類が消えるのを防ぐためのもの。
意図して大きく減らすときは値を下げるか 0 にして外す(外したことは定義に残る)。
...
**管理画面(`/admin` の「集める」)からも足せる**。一覧には相手・間隔・次にいつ・長期記憶の件数・
前回の結果が並び、行ごとに「有効にする / 止める」「試しに集めて差分を見る」
「いま集めて焼く」「削除」がある。整理(作り直し)の行は名前の横にそう出て、
前回の欄には消えた件数と見出しが並ぶ。
...
画面は知識の 2 層をそのまま節に分けてあります。

**短期記憶(覚えたこと)** が先に出ます。[notes](#notes唯一書き込めるソースの-rest) の節で、
...
その下は**AI への依頼**(`/admin#ai-history`)です。会話・絵・音・動画・声のどれでも、
**成功も失敗も**新しい順に並びます(日時・依頼の種類・相手・結果・中身)。成功にはトークン、
