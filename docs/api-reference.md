...
`_chiezo_consolidated` = [長期記憶へ移し終えた](#固化短期記憶--長期記憶)、`_chiezo_tombstone` = 長期記憶から落とす墓標。
...
**定義は機械が書き換えるものの置き場**(`state/machine.db`。`app/machine_store.py`)に
入る。かつては短期記憶(`CHIEZO_NOTES_DIR`)に 1 件のメモとして置いていたが、あそこは
人と AI が読み書きする場所なので、混ぜると**人が消せてしまう**・**固化の対象に紛れる**・
...
- **細かく切れすぎた区画は隣とまとめる。** 割り直しの引き金は「育った」と
  ...
  **まとめられるのは「幅」で表した区画だけ**(年代の帯・見出しの範囲・矩形)——
  ...
  規則も変わらない。**幅で表していないものは対象外**で、分類の名前(国名)とタグが
  それにあたる —— 2 つの名前は 1 つの名前で表せない。1 人しかいない分類は
  その 1 人で 1 区画のまま残る
...
名前だけ並べても「どう書き換わったか」は読めないので、**いまの世代と 1 つ前の世代の差分**
(本文は行単位、タグは出入りを別に)を 1 枚で出す。**比べられるのは 1 つ前まで** ——
ブルーグリーンが残す世代がそこまでなので、古い回の行から来ても出るのは最新の焼き直しの
ぶんになる。どの世代どうしを比べたかは画面に書いてある。
...
**印を付ける専用の口はありません。** `_chiezo_consolidate` はただのタグなので、
...
**判定を AI に任せられます。** 「短期記憶を順に見て、残す価値があるものに印を
...
**焼き上がった時点で印が変わります。** 長期側へ移せたメモは `_chiezo_consolidate` から
`_chiezo_consolidated` になり、`recall` の既定から外れます(長期側に反映されたことを
確かめてから付くので、移っていないのに印だけ付くことはありません)。押して回る操作では
なく、Chiezo が長期側の変化に気づいた時点で自分から動きます。

**「片付ける」(`POST /v1/memory/sweep`)は、その印が付いたメモを短期記憶から消します。**
短期記憶は「思い出す」ための場所なので、移した控えが残り続けると同じ内容が 2 か所に
積み上がります。**消えるのは控えのほうだけ**で、移った先には同じ見出しの文書が残ります
(**取り消せません**)。

素材は**前世代の長期記憶 + 印の付いたメモ**なので、1 つの流れに 3 つが乗ります。

| したいこと | 短期記憶ですること |
|---|---|
| 追加 | `_chiezo_consolidate` を付ける |
| 更新 | 長期側と同じ見出しのメモを直して `_chiezo_consolidate` を付ける(焼くとき短期側が勝つ) |
| 削除 | `_chiezo_consolidate` と `_chiezo_tombstone` の両方を付ける(墓標。長期側から落ち、墓標自体は焼かれない) |
...
- **短期記憶から移す(固化)** — [固化](#固化短期記憶--長期記憶)の待ち行列。
  長期側の件数と「固化を待っているメモ」の数が出て、「固化する」で取り込みが起きます。
  焼き上がると印は自分で変わるので、「片付ける」が消すのは**移し終えたメモ**です
  (短期記憶から消える。長期側には残る)。**待ち行列が空のときは「固化する」は
  押せません**(素材が無いと `/v1/memory/fetch` が 409 で断るため)
...
**やること層には置いていません。** あちらは認証なしで外に開く面なので、押せば枠を使う操作は載せません（読むのと印を付けるところまで）。
