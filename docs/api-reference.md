...
覚えたことのうち**確定したもの**を、テーマごとに読み取り専用のソースへ焼く仕組みです。
焼いた先は普通のソースなので、`/v1/<テーマ名>/search|doc|filter|tags` で引けます。
なぜこの形かは[設計メモ](design-notes.md#長期記憶と短期記憶を分ける)にあります。

まずテーマを定義します。名前はそのままソース名になるので英数字とアンダースコアだけです。
`tags` に挙げたタグを**全部**持つメモが、そのテーマの対象になります。

```bash
curl -s "$BASE/v1/memory/themes" \
  -H 'Content-Type: application/json' \
  -d '{"name": "rules", "label": "決まりごと", "tags": "rule"}'
```
...
素材は**前世代の長期記憶 + 未固化のメモ**なので、1 本の流れに 3 つが乗ります。

| したいこと | 短期記憶ですること |
|---|---|
| 追加 | そのタグを付けて書く |
| 更新 | 固化済みのメモを直す(`PATCH`。`固化` の印は自動で外れ、次の固化で長期側が置き換わる) |
| 削除 | 同じ見出しに `削除` タグを付ける(墓標。長期側から落ち、墓標自体は焼かれない) |
...
| 口 | 用途 |
|---|---|
| `GET /v1/memory/themes` | テーマ一覧(未固化の件数・長期側の件数つき) |
| `POST /v1/memory/themes` | テーマを足す(`name` / `label` / `tags`) |
| `DELETE /v1/memory/themes/{name}` | 定義を消す(**焼いた長期記憶は残る**) |
| `POST /v1/memory/themes/{name}/sweep` | 焼き上がりを確かめて短期側に印を付ける |
| `GET /v1/memory/sources`・`/v1/memory/fetch?source=` | 取り込み側が読む口(プラグインの契約) |
...
- **短期記憶から移す(固化)** — [固化](#固化短期記憶--長期記憶)のテーマ一覧。テーマごとに
  未固化の件数と長期側の件数が出て、「固化する」で取り込みが起き、「片付ける」で短期側に印が付きます
