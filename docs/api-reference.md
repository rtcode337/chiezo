...
からで、そこを埋めるのが `covered`(どこを終えたか)。

- プロンプトに `{covered}` を入れると、**回り終えたものの一覧**が差し込まれる
- AI は返り値に `covered`(今回回り終えた範囲の配列)を足す。**舐める収集でなければ
  書かなくてよい**(書かない収集は今までどおり動く)
- Chiezo は返った文字列を**回った順に積む**だけで、**それが地域名か人物名かは知らない**
  —— 意味は依頼した側のプロンプトが持つ(抽出の指定と同じ線)

```bash
curl -s "$BASE/v1/collect" -H 'Content-Type: application/json' -d '{
  "name": "tl_lunch",
  "description": "全国のランチ",
  "prompt": "いま集める範囲: {cursor}\n{covered}\nまだ回っていない市区町村へ進み、その範囲の店を可能な限り集める。\n全部回り終えていたら、精度を上げて最初から回り直す。\ncovered には今回回り終えた市区町村を、next_cursor には次に回る市区町村を入れる。",
  "interval_minutes": 180,
  "requested_by": "travel-log"
}'

# どこまで終えたか(1 件ぶんの口にだけ中身が入る)
curl -s "$BASE/v1/collect/tl_lunch" | jq '.covered, .covered_count'

# 最初から回り直す(2 周目を粗いまま繰り返させず、精度を上げ直したいとき)
curl -s -X PATCH "$BASE/v1/collect/tl_lunch" -H 'Content-Type: application/json' -d '{"covered": []}'
```

**一覧の口(`GET /v1/collect`)には中身を載せない**(`covered_count` だけ)。
積み上がると 1 件で数百 KB になり、収集の数だけ倍になるため。

**大きさは件数ではなく文字数で見る**(積む側 20 万字 / 差し込み 2 万字)。
何件まで回るかを決めるのは依頼した側のプロンプトで、Chiezo が持つのは
「際限なく太らない」歯止めだけ —— 全国を市区町村で舐めても 2 万字ほどなので、
積む側は**普通の使い方では当たらない**。差し込みのほうは 1 回ぶんの本文に収める
必要があるので当たりうるが、**載せきれないぶんは件数で伝える** —— 黙って切ると、
AI からは「そこまでしか回っていない」ように見えて、既に回ったところへ戻る。
