...
# 1 帯)は、**他の分類と寄せ集めて 1 区画にする**(`_pooled`)。
...
#
# **大きさでは絞らない。** 「5 人以下だけ」にしていた頃は、1 帯のままの国が
# いくつも独りで残っていた —— 年が全部で 1 帯なら受け持ちは「その国の全員」で、
# 何人いても寄せて構わない。入るかどうかは件数の上限(`target` × `MERGE_RATIO`)と
# 並べられる値の数(`MAX_BAND_VALUES`)が決める。
...
def _pooled(spec: dict, partitions: list[dict], limit: float) -> list[dict]:
    ...
    **漏れを問う**という区画の眼目が成り立たない。
    ...
    寄せ集める相手は 2 つとも満たすものだけ:

    - **数の軸で 1 つも割れていない**(その分類ぜんぶで 1 帯)。割れている分類を
      混ぜると、「この範囲の全員」が並ばなくなる —— 別の帯にいる人を漏れとして挙げる。
      **大きさでは絞らない** —— 年が全部で 1 帯なら、その分類は「範囲で区切って
      いない」ので、**入るなら寄せてよい**。件数の上限(`limit`)で足りる
