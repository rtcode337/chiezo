...
なぜこの面にあるか。 見比べはもともとやること層の画面だったが、あれはタスクの
入れ物で、**見比べているのは生成物** —— 相手・鍵・使用量・依頼の履歴と同じ
「AI に頼んだこと」の面にある。設定を見に来た人が、そのまま出来たものを見て選べる。

決めごと:

- **生成の口は置かない**。あるのは読むのと印を付けるところまで(頼む口は
  `views/media_ask.py` が別に持つ)
