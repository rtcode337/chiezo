...
- ツールの実体は `app/main.py` の REST エンドポイント関数そのもの。MCP 用に処理を
  ...
  Python から直接呼ぶときは全パラメータを明示的に渡す必要がある(省略すると Query
  インスタンスがそのまま値として渡り、`if tag:` が常に真になるような静かな誤動作になる)。
  `tests/test_mcp.py` がシグネチャを突き合わせて渡し漏れを落とす。
- MCPServer は同期のツール関数をイベントループ上で直接呼ぶ(非同期関数だけ await する)。
...
def build_mcp(app: FastAPI, with_media: bool = True, with_writes: bool = True) -> MCPServer:
    ...
    **二段目は道具そのもの**(`_refuse_bridge`)。接続先は設定で、**1 か所間違えれば
    ...
    @mcp.tool(annotations=READ_ONLY, description=(
        ...
        "**消えたもの(_chiezo_removed が付いたもの)も返る** —— 何を外したのかが"
...
def _register_image_tools(mcp: MCPServer) -> None:
    """絵を描く道具。知識を引く道具とは別の仕事だが、MCP の登録先を増やさないために
    同じサーバーに載せている。
    ...
    @mcp.tool(description=(
        ...
        "backend は image_backends で選べる相手を確認できる"
        "(既定は一覧の先頭 = 出来のよい相手。枠を使いたくないときは comfyui を名指しする)。"
    ...
    @mcp.tool(description=(
        ...
        "group に同じ名前を付けて何案か頼むと、管理画面の「見比べ」に並んで"
...
def _register_audio_tools(mcp: MCPServer) -> None:
    ...
    @mcp.tool(description=(
        ...
        "(audio_backends で確かめる。既定は一覧の先頭 = 出来のよい相手。"
        "枠を使いたくないときは comfyui を名指しする)。"
    ...
    @mcp.tool(annotations=READ_ONLY, description=(
        ...
        "管理画面の「見比べ」で聴き比べ・見比べたうえで採用が押される。"
...
def _register_video_tools(mcp: MCPServer) -> None:
    ...
    @mcp.tool(description=(
        ...
        "backend は video_backends で選べる相手を確認できる"
        "(既定は一覧の先頭 = 出来のよい相手。枠を使いたくないときは comfyui を名指しする)。"
