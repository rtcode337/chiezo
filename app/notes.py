...
# **機械が意味を持たせるタグには `_chiezo_` を前置きする。** 人が付けるタグ
# (`決定`・`画家` など)と同じ並びに、システムしか読まない印が混じると見分けられない。
# 人が同じ語をふつうのタグとして使ったときにぶつかるのも防ぐ。
#
# **先頭の `_` は「設定のためのもの」の印。** タグは名前順に並ぶので、頭に付けると
# 機械の印がひとかたまりで先頭に寄る —— 人が付けたタグの中に紛れない。
#
# **コロンは使わない。** `年代:1840-1926` のような「鍵:値」が既にあり(区画がその形で
# 読む)、`chiezo:` にすると鍵に見えてしまう。
#
# **値は英語にする。** 機械しか読まない印なので、日本語だと人が付けたタグに見える。
TAG_PREFIX = "_chiezo_"

# 長期記憶から同じ見出しの文書を落とすための墓標。本文ではなく「消す」という**指示**を
# 運ぶメモなので、焼くときに対象ごと落とす。集める層が使う(`app/collect.py`)。
TOMBSTONE_TAG = f"{TAG_PREFIX}tombstone"

# **消えたもの**の印。上の墓標が「消してくれ」という指示なのに対し、こちらは
# 「これは消えたもの」という状態で、文書に付いたまま残る。
#
# **既定では読み口から返さない**(`search` / `doc` / `filter` / `titles` / `recent` /
# `random`)。読む側が全員「この印を除く」を覚えていなくても、消したものが出てこない ——
# 新しい読み手を書くたびに同じ約束を思い出す必要があるのは、いつか必ず抜ける。
# 含めたいときだけ `include_removed` を渡す。
REMOVED_TAG = f"{TAG_PREFIX}removed"

# **消えたものを戻す**指示。墓標と同じく状態ではなく指示で、文書には残さない。
# 集める層だけが使う(`app/collect.py`)。
#
# 戻すのをわざわざ印にするのは、**「消えた印を外して返す」を戻す操作にしていた頃、
# 戻すつもりの無い返りで戻っていた**から —— AI は返す 1 件のタグを書き直すので、
# 消えた印を写し忘れただけで戻る(閉店で消した店を、同じ区画の別の依頼の回が
# 説明を書き直して返し、生き返らせた。本番で起きた)。
RESTORE_TAG = f"{TAG_PREFIX}restore"

# **まだ AI が目を通していない**印。機械で入ったもの(フィード・抽出)に付き、
# 整理の回がその 1 件を読んだ時点で外れる。
#
# 付ける理由は、機械で入るものに**宣伝も的外れも混ざる**から —— 整理が回るまでの
# あいだ、それが読者の画面に並んでいた。かといって入れないわけにはいかない
# (入れなければ、整理が何を読めばよいのか分からない)。
#
# **消したら戻ってくる**ので、精査済みを「消す」では表せない。機械で引く回は指定を
# 回すだけで毎回全件返すし、フィードも同じ見出しが流れてくる —— 重複の鍵で弾くのが
# 唯一の防波堤で、その鍵は残っていなければ働かない(本番で、外した 1 件を次の回が
# 連れ戻した)。だから**消さずに印を変える**。
UNREVIEWED_TAG = f"{TAG_PREFIX}unreviewed"

# **読み口が既定で外す印**。読む側が全員「この印を除く」を覚えていなくても
# 出てこないようにするためのもので、**1 つの並びに持つ** —— 印ごとに引数を増やすと、
# 新しい読み手が片方だけ思い出す(いつか必ず抜ける、を 2 倍にするだけ)。
HIDDEN_TAGS = (REMOVED_TAG, UNREVIEWED_TAG)

CANONICAL_TAGS: dict[str, str] = {
    "task": "作業。タスク画面に並ぶ。状態のタグが無いものは未着手",
    "着手中": "task のうち依頼済みで動作確認待ちのもの",
    "完了": "task のうち終わったもの",
    "難所": "task のうち直すのが大変そうなもの(状態とは別軸)",
    ...
    REMOVED_TAG: "消えたもの。既定では検索に出ない(include_removed で含められる)",
    UNREVIEWED_TAG: "まだ AI が目を通していないもの。既定では検索に出ない"
                    "(整理の回が読むと外れる)",
}
...
def count() -> int | None:
    ...
    画面だけではない —— MCP の `remember` も取り込みも同じ DB へ書くので、
