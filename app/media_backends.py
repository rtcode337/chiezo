...
#
# **上限は実測から取る。** 600 秒にしていた頃、成功した 9 件は 140〜575 秒
# (中央 316 秒)で、**いちばん重いものが上限の 96% に届いていた** ——
# 余裕が 4% では、少し重い依頼が落ちて当たり前になる。
#
# 時間を食っているのは相手がエージェントであること自体で、読んで・道具を使って・
# 描く、をひと通り踏む。とくに**元の絵を渡すと倍近くかかる**
# (参考なし 中央 316 秒 / 参考あり 中央 545 秒)—— 絵を取りに行って見るぶんが乗る。
# 姿勢の見本を渡す使い方では常に後者になるので、そちらを基準に取る。
BRIDGE_IMAGE_TIMEOUT = float(
    os.environ.get("CHIEZO_BRIDGE_IMAGE_TIMEOUT", "1200") or 1200
) + 30
