  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **直し方は 1 つではない**(`Collection.sweeps` / `collect.Sweep`)。育てる収集には
      **ざっと全体を拾って訂正する巡回**と、**少数をじっくり調べる巡回**の 2 つが要る ——
      ...
      - **受け持ちの範囲は、鍵のままでは伝えない**(`describe` に台帳を渡す)。
        **値の軸には上限も下限も無い**(指定できるのは区切り方だけ)ので、端の区画は
        割ったときの値より外も引き受ける。見出しの区画は次の始まりの手前まで。
        鍵どおりに書くと**引き受けているのに誰も探しに行かない範囲**ができるので、
        **端は開いたまま「以上」「以下」で言い**(「1699 以下」「1800 以上」)、
    ...
    - **端から端まで舐める収集には、カーソルのほかに「どこを見るか」の一覧が要る**
      ...
      - **細かく切れすぎた区画は、割り直さない回でも隣とまとめる**
        ...
        - **母集団の小さい分類は残る** —— 1 人しかいない国はその 1 人で 1 区画に
          なるが、まとめる相手が同じ分類に居ない。またいでまとめるとその区画が
          どの範囲なのかを言えなくなり、漏れを問うこと自体が成り立たなくなる
