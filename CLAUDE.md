...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **集め方は 2 つ**(`mode`。既定は足すほう)。違うのは**素材の作り方と、要る守り**で、
      定義・時計・AI の呼び出し・焼く経路・画面はそのまま共有する ——
      別の層にすると、この 6 つを二重に持つことになる:
      - `append`(集める)…… 前世代 + 今回のぶん。前世代は必ず残るので、失敗しても壊れない
      - `rebuild`(整理する)…… 今回のぶん**だけ**が新しい全体になる。返らなかった見出しは
        消える —— **整理そのものが目的なので、消えること自体は正しい振る舞い**。
        既にある分類をやり直す・重複をまとめる・言い回しを揃える、という育て方のためのもの
    - **作り直しには `{current}` を必須にする**(`check_prompt`。無ければ作る時点で 400)。
      ここへ今ある内容が差し込まれる。**無いまま作り直させない** —— AI は今の内容を
      知らないまま「全体」を答えることになり、返らなかったものが全部消える。
      **切るときは切ったと本文に書く**(`render_material`)—— 黙って切ると、見えなかった
      ぶんを「無かったもの」として落とした答えが返る
    - **減りすぎたら焼く前に断る**(`shrink_blocked`。既定は前世代の 5 割)。
      **ここが後戻りできる最後の地点** —— 通すと次の世代が焼き上がり、戻すには世代を
      巻き戻すしかなくなる。意図して減らすときは `keep_ratio` を下げるか 0 にして外す
      (外したことが定義に残る)。**足すほうには要らない**(そもそも減らない)
    ...
    - **定義を消しても焼いたものは残る**(`remove`)。長期記憶へ書けるのは ingest だけ
      なので、この層から消す手段はそもそも無い —— 消すのは別の意思決定である
