...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/usage.py` / `app/usage_store.py` — **各 AI の使用量**(`/v1/ai/usage`・管理画面の
    ...
    - **相手が言う枠**(`quota`)…… 相手の勘定なので**残りが分かる**が、**聞ける相手が限られる**。
      ...
      **そのあと一度「出せない側」に戻していたが、それも古かった** —— 当時は
      締めの集計しか返らなかった(`Total cost: $0.0000` / `num_turns` も 0)のが、
      CLI の版が上がってパネルが `result` に入るようになった(実測の文面は
      `app/providers.py`)。**戻る時刻に年が書かれていない**ので、ブリッジが
