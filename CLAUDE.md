...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **消すのは墓標で明示したときだけ**(`_is_tombstone`。`notes.TOMBSTONE_TAG` =
      固化とまったく同じ契約)。消したい見出しで `tags` にその印を入れて返す。
    ...
    - **誰に頼むかを、名前で外へ出せる**(`app/workers.py` / `Sweep.worker` /
      ...
      - **どれも詰まっていたらその回は走らせない**(`main._first_with_room`)。
        予定も進めないので、窓が明ければ次の周で走る。**巡回に書いてある相手へは
        落ちない** —— 落ちると、避けたかった相手に頼むことがある
      - **相手が None なのは 2 通りある**(`main._worker_step` が 2 つ目を返す理由)。
        ワーカーを使わない回(巡回の指定で走る)と、どれも詰まっている回(見送る)で、
        次にすることが逆になる
  ...
  - `app/memory.py` — **「固める」層(記憶の固化。短期記憶 → 長期記憶)**。
    ...
    - **世代の日付は秒まで入れる**(`_dump_date`)。世代ファイルは `<source>-<日付>.db` で
      ...
      `-` が世代ファイル名の区切りそのものだから
    - **印を付けるのは焼いた後**(`mark_consolidated`)。条件は「意図どおり長期側へ
      反映されていること」—— 通常のメモは同じ見出しが長期側にあること、墓標は
      無くなっていること。**焼く前に呼んでも何も起きない**ので、移っていないのに
      印だけ付く事故が起きない。
      **押して回る操作にはしない**(`catch_up` を `main.refresh_sources` が呼ぶ)——
      長期側が変わったという合図をいちばん早く受け取るのがそこで、固化が終わったか
      どうかは他に知る手立てが無い(焼くのは別のプロセス)。人が押す形にしていた頃は、
      焼けているのに印が「まだ移していない」のまま残り、次の固化で同じものをもう一度
      焼いていた。**待っているものが無ければ何もしない** —— どのソースを焼き直しても
      呼ばれるので、相手が居ないうちから長期側の見出しを全部読むわけにいかない
    - **「片付ける」(`sweep`)は、移し終えたメモを短期記憶から消す。**
      短期記憶は「思い出す」ための場所なので、移した控えが残り続けると同じ内容が
      2 か所に積み上がり、直すときにどちらが正か言えなくなる。**消えるのは控えの
      ほうだけ**(移った先には同じ見出しの文書がある)。**消すのは印が付いている
      ものだけ**で、その印は長期側に反映されたことを確かめてから付く ——
      **取り消せない**ので、画面には件数つきの確認を出す
  ...
  - `app/tasks.py` — **「やること」層(タスク・プロジェクト・ルール)**。cc-tasks から
    ...
    - **状態のタグが無いものが「未着手」**。一番多い状態にタグを増やさずに済み、
      既存のメモに手を入れなくてよくなる
    - **完了にしたタスクは `_chiezo_consolidate` にもなる**(`_task_tags`)。片付いたタスクは
      「やった記録」として残す価値があり、しかも**待ち行列からは外れてほしい**もので、
      固化はその 2 つを同時にやる(長期記憶へ移し、`recall` の既定から外す)。
      完了のたびに人が印を付けて回るなら、結局そこが抜ける。**戻したら外す**が、
      **すでに焼き終えたもの(`_chiezo_consolidated`)には触らない** —— 長期側に入っているという事実は
      こちらの都合で書き換えるものではない
    - **長期記憶へ移したものも一覧に出す**(`_frozen_rows` / `Task.frozen` /
      `Rule.frozen`)。固化は「もう触らないものを短期から下ろす」操作なので、
      下ろした瞬間に画面から消えると、片付いたタスクも寝かせたルールも見えなくなる
      —— **移した先に残っているのに読めないのでは、移す気にならない**。
      ルールの連結(`combined`)も同じで、逃がした途端に効かなくなっては困る。
      **読めるが直せない** —— 中身は長期側にあり、短期へ引き出す口はまだ無いので、
      書き換えの口は `refuse_if_frozen` で断る(黙って何も起きないより理由を言う)。
      **`doc_id` は符号を反転させて持つ** —— 短期と長期は別の DB で採番しているので
      そのままでは重なり、画面の鍵にも使えないし、書き換えの口に渡ったときに
      別のものを直してしまう。**並び替えの対象からは外す**(順番は短期側の `extra` に
      書くので動かせない。一覧の末尾に固める)
    - **ルールも長期記憶へ逃がせる**(`mark_rule_for_long_term`)。
      **無効にするのとは違う** —— 無効(`TAG_RULE_DISABLED`)は連結から外す =
      もう効かせない、の意味。こちらは**効かせたまま直す対象から下ろす**もので、
