...
- `api/` — **chiezo-api**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/settings_store.py` — 管理画面から入れた設定の置き場。**`CHIEZO_STATE_DIR` が
    ...
    **既定は有効だが、それで勝手に動き出すことは無い**(相手が全部既定 off なので)。
    ...
    増えない)だが、**画面には二度と出さない**。
  - `app/views/ai_settings.py` — 管理画面の「話す相手」節と、on/off・API キーの受け口。
...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-api` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - `/v1/media/backends`(GET) / `/v1/media/image`(POST) / `/v1/media/jobs/{id}`(GET)
    ...
    **待たない**(job を返し `image_status` で引く。生成は数秒〜数分で、待つと呼び出し側が
    ...
    (あちらは CLI ブリッジが読み取り専用でマウントしている)。**配信はパスを解いてから**
    ...
    そのまま使える)。**on/off も「話す相手」と共通** —— そちらで無効にした相手には
    絵も描かせない(`media_backends.unusable_reason`)。鍵を持っている相手を止めたのに
  ...
  - 管理画面には**「絵を描く相手」節**を出す(`app/views/ai_settings.py`)。
    自前の GPU(ComfyUI)は「話す相手」ではないので上の表に出てこず、状態を見る場所が
    無くなるため。**自前の GPU の on/off と「接続を試す」はこの節に置く**
    (`owns_toggle`)。外部サービスは「話す相手」と共通で、**同じものを 2 か所から
    切り替えさせない** —— どちらが効いているのか画面から読めなくなる。
    「接続を試す」は繋がるかに加えて**チェックポイントの有無まで見る**
    (立っていてもモデルが無ければ 1 枚も描けない)。絵も描ける相手には上の表に印を付ける
    (同じ鍵で 2 つの用途に使えること、止めると絵も止まることが画面から読めるように)
  - **生成する CLAUDE.md ブロックにも画像の節を足す**(`claude_config.build_block` の
    `image=True`)。**MCP を登録していて、かつこのサーバーで絵を描けるときだけ** ——
    呼べない道具を勧めない
