...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **端から端まで舐める収集には、カーソルのほかに「どこを終えたか」が要る**
      (`Collection.covered`)。`{covered}` に回り終えたものの一覧が差し込まれ、AI が
      返り値の `covered` で今回ぶんを名乗る。**カーソル 1 本では足りない** ——
      あれは「次はどこ」しか持てないので、**全部を舐めきったかも、取りこぼしがどこかも
      分からない**。一覧があると「まだのところへ進め」「全部回り終えたら精度を上げて
      最初から回り直せ」が書け、外のアプリも「この範囲はもう集めてある」を判断できる。
      **中身が何かは Chiezo は知らない**(地域名でも人物名でもよい)—— 返った文字列を
      積んで見せ返すだけで、意味は依頼した側のプロンプトが持つ(抽出の指定と同じ線)。
      **積む順は回った順**(同じものを再び返しても後ろへ動かさない)。
      **大きさは件数ではなく文字数で見る**(`MAX_COVERED_CHARS` / 差し込みは
      `MAX_COVERED_PROMPT_CHARS`)—— このファイルの「件数は大きさの代理にならない」と
      同じ理由で、素材の差し込み(`MAX_MATERIAL_CHARS`)と流儀を揃えてある。
      **何件まで回るかを Chiezo が決めない**(どこまで細かく回るかは依頼した側の
      プロンプトが決めること)—— 持つのは定義のメモが際限なく太らない歯止めだけで、
      全国を市区町村で舐めても 2 万字ほどなので**普通の使い方では当たらない**。
      差し込みのほうは 1 回ぶんの本文に収める必要があるので当たりうるが、
      **載せきれないぶんは件数で伝える** —— 黙って切ると、AI からは
      「そこまでしか回っていない」ように見えて、既に回ったところへ戻る。
      **一覧の口(`GET /v1/collect`)には中身を載せない**(件数だけ)—— 上限まで積むと
      1 件で数百 KB になり、収集の数だけ倍になる。中身は 1 件ぶんの口から取る。
