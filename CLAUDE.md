...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **順の無いソースに `limit` を書かせない**(`MAX_ROWS` = 50 万)。同じ種別の地物に
      ...
      地図辞典 1 国ぶんが丸ごと入る高さにする** —— 日本の飲食店だけで 41 万件あり、
      ここで切ると「全部を集めて精査する」という収集の種類そのものが嘘になる。
      **本当の天井は素材のほう**(`collect.MAX_MATERIAL_BYTES` = 192 MB)。
      実測で 1 件 256 バイト・引いたときのメモリが 1 件 2.6 KB なので、
      50 万件で素材 130 MB・取り込み時 1.6 GB。**前世代も同時に載る**ので、
      定常では倍を見ておく
