...
- `api/` — **chiezo-api**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/main.py` — ルーティング(/, /healthz, /v1/sources,
    ...
    - **`total` はタグだけで絞るときは `doc_tags` から数える**(`docs` 側で
      `COUNT(*) … WHERE doc_id IN (…)` にすると該当 doc_id ごとに 41GB の `docs` へ
      rowid 検索が飛び、jawiki の「存命人物」25 万件で 33 秒。転置表だけなら 0.01 秒)
    - **並び替えも該当が多いと同じ理由で詰む**。既定の計画は該当文書を全部読んでから
      並べるので費用が件数に比例する(同 25 万件で `limit=1` でも 33 秒)。`rank_index_hint()`
      が総件数を見て、多いときだけ `INDEXED BY idx_docs_rank` を付け、並び順を持つ索引を
      上位 N 件で打ち切る計画に倒す(0.05 秒)。**少ないときに付けてはいけない**
      (索引を端まで走査しても N 件見つからない。1 件のタグで 0.001 秒 → 0.044 秒)。
      **タグ以外の条件が混ざるときも付けてはいけない**(`feature`/`area`/`lat`/`lon` は
      VIRTUAL な生成列なので、判定に行本体が要り 150 万行を読むことになる)
