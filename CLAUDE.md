...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/views/ai_failures.py` — 管理画面の「AI 依頼の失敗」節。**会話と生成を
    分けない** —— どちらで落ちたか分かっていない人が探せなくなるので、種類は列で示して
    1 枚に並べる。控え自体は前からあったが、読む手段が `GET /v1/ai/failures` の
    JSON しか無かった(**失敗に気づく人はたいていその場に居合わせていない**)。
...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-app` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - **「集める」の節は長期記憶の中(h3。固化の上)**。焼き先が長期記憶(`corpus/`)
    そのものなので、位置と実態が一致している。行には**待ち行列の件数と長期記憶の件数**を
