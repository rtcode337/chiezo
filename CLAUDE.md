  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **依頼文から指定を書かせる口がある**(`POST /v1/collect/draft-extract` / 画面の
      ...
      **頼んだ件数に届かなければ、実在するタグを文書数つきで見せて 1 度だけ選び直させる** ——
      0 件だけが失敗ではない。「画家」のような一般名は実在するが数件しか付いておらず、
      欲しいものは「19世紀フランスの画家」の側にある(実際にそう書かれた)。
    ...
    - **外すタグも書ける**(`not_tag`)。**カテゴリは持ち主の職業を選ばない** ——
      「20 世紀アメリカ合衆国の画家」には絵も描く俳優や作家が入っていて、人気の順に
      取ると本来欲しい人より先に並ぶ(実際に上位 30 人へ俳優が 2 人入った)。
      機械で外せるのはカテゴリまでで、「絵で知られているか」の見分けは AI の側の仕事
