  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **直し方は 1 つではない**(`Collection.sweeps` / `collect.Sweep`)。育てる収集には
      **ざっと全体を拾って訂正する巡回**と、**少数をじっくり調べる巡回**の 2 つが要る ——
      ...
      - **区画ごとに AI を 1 回ずつ呼ぶ**(`MAX_PARTITIONS_PER_RUN` で頭打ち。いまは 6)。
    ...
    - **割り込みは巡回とは別の道**(`collect.Focus` / `POST /v1/collect/{name}/focus`)。
      ...
      - **区画の数が天井(`MAX_PARTITIONS`)に当たったら、削るのは細かさであって
        範囲ではない**(`partition._fits`)。打ち切ると、**打ち切った先の母集団が
        どの区画にも入らず、どの巡回にも回ってこない** —— 本番で 68 万件を目安 150 で
        割ろうとして 2,000 区画(1 区画 200 未満)で止まり、40 万件ぶんの範囲が
        台帳から消えた。目安を上げれば 1 回に見る量が増えるだけで済む。
        **余裕は 2 倍で見る** —— 二分は割り切れないので、できる区画は目安の半分から
        等倍に散らばる。実測: 68 万件・目安 150 → 1,024 区画・1 区画 670・合計は一致
