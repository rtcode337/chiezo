...
- **集める / 育てる** — `app/collect.py` が、**まとまったダンプの無いもの**を AI に
  ...
  **巡回は 1 本とは限らない**(`Sweep`)。ざっと全体を拾うものと、少数をじっくり調べるものを
  ...
  進み具合には触らない)。**1 回ごとの変更履歴が残る**(`app/collect_log.py`)。
...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **差分を控える**(`last_removed` / `last_removed_titles`)。件数だけでは
      ...
      メモが太るので頭の 10 件まで
    - **焼かずに試せる**(`POST /v1/collect/{name}/preview` / 画面の「試しに集めて差分を見る」)。
...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-app` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - **見出しは 3 つの面に畳んである**(`知識(溜めて引く)` / `AI と鍵(貸し出すもの)` /
    ...
    **長期記憶の件数**で、押すボタンは**「いま集めて焼く」の 1 つだけ** ——
    集めるのも焼くのも取り込みの中で起きるので、分ける段が無い
