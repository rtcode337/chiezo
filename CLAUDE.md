...
この 3 層と 1 層は**同じものの 2 つの側**でもある。ためた知識(公開ダンプ)と固めた知識
(自分のメモ)はどちらも `corpus/` の読み取り専用ソースで、**引く口はまったく同じ**。
違うのは素材が外にあるか手元にあるかだけ。**画面では「長期記憶」「短期記憶」と呼ぶ**
(`/admin` の 2 節)。
...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/memory.py` — **「固める」層(記憶の固化。短期記憶 → 長期記憶)**。
    ...
    - **テーマの置き場は `state/memory.db`**。`settings.db` に相乗りしないのは、
      あちらを CLI ブリッジのコンテナが読み取り専用でマウントするため(使用量・
      失敗ログを別ファイルにしてあるのと同じ判断)
    - **テーマ名はそのままソース名**なので、取り込み側と同じ制限(`[A-Za-z0-9_]`)を
      課し、**登録済みのソース名は弾く** —— 通すと焼いた瞬間に別のソースを置き換える
...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-app` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - `/admin`・`/admin/osm`・`/admin/wikipedia`(GET) — 簡易 HTML 管理画面(画面に何が出るかは
    ...
    **固化(`app/memory.py`)のテーマ一覧は長期記憶の節の中に、「未初期化データの初期化」
    の前**に置く —— どちらも「長期記憶に何を入れるか」の操作で、素材が短期記憶か
    外のダンプかが違うだけ。焼くこと自体は普通の取り込みなので、ボタンの行き先は
    初期化・再構築と同じ。
  ...
  - `/v1/memory/themes`(GET / POST)・`/v1/memory/themes/{name}`(DELETE)・
  ...
  - `/admin/memory/themes`(POST)・`/admin/memory/{name}/sweep|delete`(POST) —
    管理画面のフォームから叩く口(303 で `/admin#consolidation` へ戻す)。
    **焼くこと自体の口は持たない** —— 普通の取り込みなので `/admin/init|rebuild` を使う
