...
- **集める** — `app/collect.py` が、**まとまったダンプの無いもの**を AI に集めさせて
  ...
  **素材が「前世代 + 新しく集めたぶん」になる**ので、ブルーグリーン(全件の作り直し)に
  乗せたまま追記として積み上がる(世代は今と 1 つ前だけ)。集めたものは焼くまで
  待ち行列(`CHIEZO_COLLECT_DIR`)に置き、notes には混ぜない —— 短期記憶が収集物で
...
- `app/` — **chiezo-app**: FastAPI + uvicorn の常駐コンテナ。起動時に `CHIEZO_DATA_DIR`(既定 `/data`)を走査し、
  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **`CHIEZO_COLLECT_DIR` が機能フラグを兼ねる**(未設定なら時計も回らない)。
    ...
    - **時計は Chiezo が持つ**(`main._run_collections`)。ホストの cron に出さないのは、
      ...
      ティックの間隔は「見に来る頻度」でしかない。**1 周に 1 件ずつ**走らせる ——
      CLI ブリッジ越しの相手は同時に 1 本しか動かないので、まとめて投げても待ち行列になる
    - **失敗しても次回の予定を入れる**(`record_result`)。入れないと、一度こけた収集が
      二度と走らなくなる。理由は定義側に控えるので、画面の「前回」の欄から読める
    - **定義を消しても溜めたものは残す**(`remove` の既定)。消すのは別の意思決定で、
      間違えたときに取り返しがつかない
...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-app` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - **見出しは 3 つの面に畳んである**(`知識(溜めて引く)` / `AI と鍵(貸し出すもの)` /
    ...
    焼き先が長期記憶(`corpus/`)そのものなので、位置と実態が一致している。行には**待ち行列の件数と長期記憶の件数**を
    並べ、「長期記憶へ焼く」は初回が `init`・2 回目以降が `rebuild`(普通の取り込みと同じ導線)
