  ...
  - `app/collect.py` — **「集める」層(AI に集めさせて溜めていく)**。
    ...
    - **外向きの道具**(`app/feeds.py` / 定義の `feed` / プロンプトの `{feed}`)。
      ...
      **AI が間違えるところと、間違えないところを分ける**(見出し・URL・日付は機械が
      ...
      - **取ってきたものをそのまま溜めない。** `{feed}` へ**参考として**差し込むだけで、
        何を溜めるかは AI が決める(自分でも web を検索し、渡されたぶんも含めて
        採否を判断する)。差し込む文にもそう書く。**生のまま入る経路は作らない** ——
        情報源として扱わせると、フィードが拾わなかったものは永遠に入らず、
        フィードが拾った宣伝記事はそのまま溜まる
