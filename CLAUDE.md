...
- **`bridge/` — CLI ブリッジ(別イメージ `ghcr.io/<owner>/chiezo-bridge`)。**
  ...
  **本体には入れない** —— `chiezo-api` が数百 MB で動く前提を崩さないため(推論を同居させないのと
  ...
  - 管理画面には**「絵と音を作る相手」節**を出す(`app/views/ai_settings.py`)。
    自前の GPU(ComfyUI)と ElevenLabs は「話す相手」ではないので上の表に出てこず、
    状態を見る場所が無くなるため。**その 2 つの鍵・on/off・「接続を試す」はこの節に置く**
    (`owns_toggle`)。「話す相手」に対応がある外部サービスはあちらと共通で、
    **同じものを 2 か所から切り替えさせない** —— どちらが効いているのか画面から読めなくなる。
    **行は相手ごとに 1 つ**にして、作れるもの(絵・音)は行の中に並べる ——
    同じ相手が 2 行に分かれると on/off がどちらに効くのか読めない。
    「接続を試す」は繋がるかに加えて**チェックポイントの有無まで見る**
    (立っていてもモデルが無ければ 1 枚も描けない)。**絵と音は別のファイルが要る**ので
    どちらが何件あるかも返す。絵や音も作れる相手には上の表に印を付ける
    (同じ鍵で 2 つの用途に使えること、止めるとそちらも止まることが画面から読めるように)
