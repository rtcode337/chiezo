...
**動画はどの相手も非同期**で、頼んだ時点では id しか返らない —— 待ち方は
...
def timeout_for(kind: str) -> float:
    """その kind を待つ上限。**動画だけ桁が違う**ので、ここで 1 つに寄せる。
...
@dataclass(frozen=True)
class SpeechRequest:
    """文章を読み上げる頼み。**`prompt` が読み上げる文章そのもの**で、絵や音のような
...
class TranscribeRequest:
    """文字起こしの頼み。**こちらだけ入力がバイト列**なので、他と口の形が違う。"""
...
def resolve_video_seconds(spec: media_providers.MediaProvider, seconds: float) -> float:
    ...
    音の `resolve_seconds` と違って**丸めない** —— 動画は受け付ける値が飛び飛びなので、
    近い値へ寄せると「6 秒で頼んだのに 8 秒が返る」になる。範囲の検査は
    `media.create_job` が先に済ませてあり、ここへ来るのは通った値だけ。

    既定を最短にしてあるのは、**間違えたときの損が小さいほう**を既定にするため
...
async def _await_remote(check, *, every: float, timeout: float, what: str):
    ...
    **動画の相手はどこも非同期**(頼んだ時点では id だけが返る)なので、待ち方は
    ここに 1 つだけ置く。**間隔を相手ごとに変えられる**ようにしてあるのは、
...
async def _comfy_execute(
    ...
    **名前は複数受け取れる** —— 動画の入れ物の名前は ComfyUI の版で変わるため
    ...
    `timeout` は待つ上限(0 なら絵と同じ)。**動画だけ桁が違う**ので外から渡せる。
    """
...
async def comfy_video_models(url: str, timeout: float = 5.0) -> list[str]:
    """置いてある**動画の**モデル(`models/diffusion_models`)。"""
...
def _comfy_video_graph(
    ...
    """Wan の text-to-video。**絵のグラフと骨格は同じ**で、違うのは潜在空間が
...
# **尺は 4 / 8 / 12 秒しか取らない**ので、`media.create_job` が一覧と突き合わせて弾く。
def _form(fields: dict) -> dict:
    """文字だけの multipart。**httpx は `files=` を渡したときだけ multipart にする**ので、
...
# Seedream、動画は Veo / Seedance)。絵も動画も**同じ作り**で、違うのはパスと本文の
# 項目だけなので、実装は 1 つで足りる。
#
# **どちらも非同期**(頼むと id だけが返る)。仕上がると署名済みの `content_url` が
# 生えるので、そこから取りに行く —— **1 時間で切れる**ので、返ってきたその場で拾う。
...
# プロンプトではなく**読み上げる文章と声**なので、`SpeechRequest` を別に持っている。
#
...
async def _elevenlabs_voice_id(spec: media_providers.MediaProvider, wanted: str) -> str:
    ...
    見つからなければ**そのまま id として渡す**(相手が知っていれば通る。こちらが
...
async def _gemini_speech(
    ...
    # **曲(Lyria)と同じ `interactions` の口**で、違うのは声の指定が付くことだけ
    body = {
        "model": model,
        "input": req.prompt,
        "response_format": {"type": "audio"},
...
async def generate_for(kind: str, backend: str, req):
    """job の kind で呼び分ける。**`media._run` が中身を知らずに回せる**ようにするため。
